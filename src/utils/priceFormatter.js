...
export const formatDepositInfo = (rent, deposit) => {
  if (!deposit || deposit === 0) return '敷金なし';
  
  const months = Math.round(deposit / rent * 10) / 10;
  if (months === Math.round(months)) {
    return `敷金${Math.round(months)}ヶ月分`;
  } else {
    return `敷金${months}ヶ月分`;
  }
};
