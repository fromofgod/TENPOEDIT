      ...
      <>
        <div className="flex items-center space-x-3">
          <button
            ...
            className="text-sm font-medium text-gray-600 hover:text-red-600 transition-colors px-3 py-2 rounded-lg hover:bg-red-50"
          >
            ログイン
          </button>
          <button
            onClick={() => handleSignIn('signup')}
            className="bg-gradient-to-r from-red-500 to-red-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:from-red-600 hover:to-red-700 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105"
          >
            新規登録
          </button>
        </div>
      ...
      <div className="relative">
        ...
        >
          <div className="relative">
            <img
              src={user.picture}
              alt={user.name}
              className="w-7 h-7 rounded-full ring-2 ring-red-100"
            />
            {isAdmin && (
        ...
        <AnimatePresence>
          {showDropdown && (
            ...
            >
              {/* User Info Header */}
              <div className="px-4 py-4 border-b border-gray-100 bg-gradient-to-r from-red-50 to-red-50">
                <div className="flex items-center space-x-3">
                  <div className="relative">
                    <img
                      src={user.picture}
                      alt={user.name}
                      className="w-12 h-12 rounded-full ring-3 ring-red-200"
                    />
                    {isAdmin && (
                      <div className="absolute -bottom-1 -right-1">
                        <SafeIcon
                          icon={FiShield}
                          className="w-5 h-5 text-purple-600 bg-white rounded-full p-1"
                        />
                      </div>
                    )}
                  </div>
