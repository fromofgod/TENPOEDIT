...
const GoogleMap = ({
      ...
      const icon = {
        url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
          <svg width="36" height="44" viewBox="0 0 36 44" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="18" y="23" text-anchor="middle" fill="${color}" font-size="11" font-weight="bold">¥</text>
          </svg>
        `)}`,
              ...
              <div class="space-y-4">
                ...
                <div class="pt-3 border-t grid grid-cols-2 gap-2">
                  <a href="#/property/${property.id}" 
                     class="block text-center bg-blue-600 text-white px-3 py-2 rounded text-sm font-medium hover:bg-blue-700 transition-colors">
                    詳細を見る
                  </a>
                  <button onclick="window.showStreetView(${position.lat},${position.lng})" 
  ...
  const handleMapSearch = () => {
    ...
    geocoder.geocode(
      { address: searchQuery, region: 'JP', language: 'ja' },
      (results, status) => {
