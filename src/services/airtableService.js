...
const coordinates=(latitude && longitude) ? {lat: latitude,lng: longitude,source: 'airtable'} : null;
...
usageArea: getAreaValue(fields), // 修正: 安全な面積取得
...
area: transformedProperty.area, // 面積の確認
...
console.log('🏢 Area fields check:',{
'使用部分面積': allRecords[0].fields['使用部分面積'],
'面積': allRecords[0].fields['面積'],
'専有面積': allRecords[0].fields['専有面積']
});
}
...
withArea: transformedProperties.filter(p=> p.area && isNumeric(p.area)).length,
byType: {}
};
...
console.log('🏢 Area validation: 面積データの安全な取得を実装済み');
...
// 面積フィールドの確認
const areaFields=[];
const areaFieldNames=['使用部分面積','面積','専有面積','建物面積','延床面積'];
areaFieldNames.forEach(fieldName=> {
if (fields[fieldName]) {
areaFields.push(fieldName);
}
});

console.log('🖼️ Image fields found:',imageFields);
console.log('🏢 Area fields found:',areaFields);
console.log('🖼️ Image order setting: 画像2が最初に表示されます');
console.log('🏢 Area parsing: 安全な面積データ取得を実装済み');
